...
object DevDataSeeder {
    ...
    suspend fun seedHotelsIfEmpty() = newSuspendedTransaction(Dispatchers.IO) {
        if (Hotels.selectAll().count() == 0L) {
            val hotels = (1..10).map { i ->
                val minPrice = Random.nextDouble(50.0, 200.0)
                val maxPrice = minPrice + Random.nextDouble(50.0, 300.0)
                val contactInfo = if (Random.nextBoolean()) mapOf(
                ...
                val tags = listOf("Luxury", "Family-Friendly", "Business", "Budget", "Boutique").shuffled().take(Random.nextInt(1, 4))
                val bookingLinks = if (Random.nextBoolean()) listOf("https://booking.com/hotel$i", "https://agoda.com/hotel$i") else emptyList()
