...
class HotelRepositoryImpl : HotelRepository {
    ...
    private fun toHotelResponse(row: ResultRow) = HotelResponse(
        id = row[Hotels.id],
    ...
    override suspend fun createHotel(
        nameVi: String, nameEn: String, slug: String, descriptionVi: String?, descriptionEn: String?,
        address: String?, city: String?, latitude: Double?, longitude: Double?, addressLink: String?,
        contact: String?, images: String?, minPrice: Double?, maxPrice: Double?, amenities: String?,
    ...
    override suspend fun getHotelById(id: Long): HotelResponse? = newSuspendedTransaction(Dispatchers.IO) {
        selectQuery.where { Hotels.id eq id }.singleOrNull()?.let(::toHotelResponse)
    }

    override suspend fun getHotelBySlug(slug: String): HotelResponse? = newSuspendedTransaction(Dispatchers.IO) {
        selectQuery.where { Hotels.slug eq slug }.singleOrNull()?.let(::toHotelResponse)
    }

    override suspend fun getHotelsByHostId(hostId: Long): List<HotelResponse> = newSuspendedTransaction(Dispatchers.IO) {
        selectQuery.where { Hotels.hostId eq hostId }.map(::toHotelResponse)
    }

    override suspend fun getHotelsByCity(city: String): List<HotelResponse> = newSuspendedTransaction(Dispatchers.IO) {
        selectQuery.where { Hotels.city eq city }.map(::toHotelResponse)
    }

    override suspend fun getHotelsByTags(tags: List<String>): List<HotelResponse> = newSuspendedTransaction(Dispatchers.IO) {
    ...
    override suspend fun updateHotel(
        id: Long, nameVi: String, nameEn: String, slug: String, descriptionVi: String?, descriptionEn: String?,
        address: String?, city: String?, latitude: Double?, longitude: Double?, addressLink: String?,
        contact: String?, images: String?, minPrice: Double?, maxPrice: Double?, amenities: String?,
        checkInTime: String?, checkOutTime: String?, cancellationPolicy: String?, childPolicy: String?,
        petPolicy: String?, tags: String?, externalBookingLinks: String?, hostId: Long?
    ): HotelResponse? = newSuspendedTransaction(Dispatchers.IO) {
        val updatedRows = Hotels.update({ Hotels.id eq id }) {
            it[this.nameVi] = nameVi
            it[this.nameEn] = nameEn
            it[this.slug] = slug
            it[this.descriptionVi] = descriptionVi
    ...
    override suspend fun deleteHotel(id: Long): Boolean = newSuspendedTransaction(Dispatchers.IO) {
        Hotels.deleteWhere { Hotels.id eq id } > 0
    }

    override suspend fun updateRating(id: Long, rating: Float, reviewCount: Int): Boolean = newSuspendedTransaction(Dispatchers.IO) {
    ...
    override suspend fun incrementViewsCount(id: Long): Boolean = newSuspendedTransaction(Dispatchers.IO) {
        Hotels.update({ Hotels.id eq id }) {
    ...
    override suspend fun incrementFavoritesCount(id: Long): Boolean = newSuspendedTransaction(Dispatchers.IO) {
        Hotels.update({ Hotels.id eq id }) {
            with(SqlExpressionBuilder) {
    ...
    override suspend fun decrementFavoritesCount(id: Long): Boolean = newSuspendedTransaction(Dispatchers.IO) {
        Hotels.update({ Hotels.id eq id }) {
            with(SqlExpressionBuilder) {
                it.update(favoritesCount, favoritesCount - 1)
            }
